...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,guessed):
        return [letter for letter in LETTERS 
                ...
                and letter not in 'AEIOU')
                or (letter not in guessed
                and letter in 'AEIOU'
                and self.prizeMoney >= 250)
               ]
    
    def getMove(self, category, obscuredPhrase, guessed):
        possibmoves = self.getPossibleLetters(guessed)
        possibmoves = sorted(possibmoves, key=lambda x:self.SORTED_FREQUENCIES.index(x), reverse=True)                   
        
        if len(possibmoves) == 0:
            return 'pass'  
                       
        if self.smartCoinFlip():
            return possibmoves[0]
               
        return random.choice(possibmoves)
